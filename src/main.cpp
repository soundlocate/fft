...
int update_buffer(double * base_buffer, double * new_data,
                  unsigned int base_count, unsigned int new_count) {
...
int init(int samples, int mic_count, int window) {
    ...
    auto finalTime = std::chrono::duration<double, std::ratio<1l, 1000l>>(
        std::chrono::high_resolution_clock::now() - planningTime);
...
int convert_output(complex * out, double * out_converted, int samples,
                   int mic_count, double threshold) {
    ...
    for(int i = 0; i < samples / 2 + 1; i++) {
        ...
        for(int j = 0; j < mic_count; j++) {
            ...
            double temp_amplitude =
                ((sqrt(t_real * t_real + t_complex * t_complex)) /
                 (0.5 * samplerate));
            t_amplitude =
                (temp_amplitude > t_amplitude) ? temp_amplitude : t_amplitude;
        }
        ...
        if(t_amplitude > threshold) {
            for(int j = 0; j < mic_count; j++) {
                ...
                double freq =
                    (double)i * ((double)samplerate / (double)samples);
                ...
                out_converted[3 * mic_count * converted_count + 3 * j + 1] =
                    phase;
                out_converted[3 * mic_count * converted_count + 3 * j + 2] =
                    amplitude;
            }
...
int main(int argc, char ** argv) {
    ...
    Client client(options.audioIp().c_str(), options.useFPGA(),
                  options.audioPort(), mic_count);
    server =
        new Server(options.fftPort(), [&mic_count](sf::TcpSocket * socket) {
    ...
    while(true) {
        ...
        STOPWATCH("fft_execute_fft", fft->execute(in, out););

        auto finalTime = std::chrono::duration<double, std::ratio<1l, 1000l>>(
            std::chrono::high_resolution_clock::now() - fftTime);

        //		std::cout << "FFT took: " << finalTime.count() << "ms" <<
        //std::endl;

        STOPWATCH("fft_convert_output",
                  converted = convert_output(out, out_converted, samples,
                                             mic_count, options.threshold()););
        ...
        STOPWATCH(
            ...
            //				  server.send(out_converted, 200 * mic_count *
            //3);
            );

        STOPWATCH("fft_recieve_data", in_new = client.receive(block_size););

        if(in_new == nullptr) {
            std::cout << "server disconnected, trying reconnect" << std::endl;

            client.reconnect(options.audioIp().c_str(), options.audioPort(),
                             mic_count);
        ...
        */
        STOPWATCH("fft_remap_buffer",
                  update_buffer(in, in_new, samples * mic_count,
                                block_size * mic_count););
